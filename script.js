  ...
  document.getElementById("dynamic-gallery").innerHTML = car.gallery
    .map(
      ...
      <figure class="gallery-item${isFeatured ? " gallery-item--featured" : ""}">
        <button type="button" class="gallery-item__trigger" aria-label="Open image in lightbox">
...
function initLightbox() {
  ...
  function getLightboxFocusableElements() {
    return [...lightbox.querySelectorAll(focusableSelector)].filter(
  ...
  function showImage(index) {
    ...
    const selectedImage = galleryTriggers[currentIndex].querySelector("img");
    if (!selectedImage) return;

    lightboxImg.src = selectedImage.src;
    lightboxImg.alt = selectedImage.alt || "Vehicle image";
  }
  ...
  galleryTriggers.forEach((trigger, i) => {
    trigger.addEventListener("click", () => openLightbox(i, trigger));
  });
